...
public class CharacterMotor : MonoBehaviour
{
    ...
    private void UpdateFunction()
    {
		...
		movement.gravity = manager.GravityAtPoint (transform.position);
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity)
    {
        ...
        if(grounded){
			jumping.jumpDir = groundNormal;
            //velocity.y = Mathf.Min(0, velocity.y) - movement.gravity.magnitude * Time.deltaTime;
			//transform.Rotate(transform.forward, Mathf.Max (Vector3.Angle (transform.up, Vector3.up), 5));
			if(transform.up != groundNormal){
				transform.up = Vector3.RotateTowards(transform.up, groundNormal, 0.05f, 0.0f);
			}
		}
        else
        {
			
			transform.up = Vector3.RotateTowards(transform.up, -movement.gravity, 0.05f, 0.0f);
			
            //velocity.y = movement.velocity.y - movement.gravity * Time.deltaTime;
			velocity = movement.velocity + movement.gravity * Time.deltaTime;
            ...
            if(jumping.jumping && jumping.holdingJumpButton)
            {
                ...
                if(Time.time < jumping.lastStartTime + jumping.extraHeight / CalculateJumpVerticalSpeed(jumping.baseHeight))
                {
                    ...
                    velocity += jumping.jumpDir * movement.gravity.magnitude * Time.deltaTime;
                }
            }
	...
	void OnDrawGizmos(){
		if(manager != null)
			Gizmos.DrawRay (transform.position, movement.gravity);
    ...
    float CalculateJumpVerticalSpeed(float targetJumpHeight)
    {
        ...
        return Mathf.Sqrt(2 * targetJumpHeight * movement.gravity.magnitude);
    }
    ...
    bool TooSteep()
    {
        return (Vector3.Angle(transform.up, groundNormal) <= Mathf.Cos(controller.slopeLimit * Mathf.Deg2Rad));
    }
